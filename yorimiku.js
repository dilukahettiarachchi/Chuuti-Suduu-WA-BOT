    ...
    const _0x547672 =['8SFTvTY','733698bByGMZ','1nqbVzC','11dOuxIL','chat','1838816QQlKFJ','992463nstgLu','4616415lmcnqf','1014PlYMnG','23639900whcpis','5450zYeATk','1020716qbYiGA'];_0x1aa5=function(){return _0x547672;};return _0x1aa5();}
        ...
        if (db.data.chats[m.chat].antilink) {
            ...
            if (budy. match(`chat.whatsapp.com`)) {
                newReply(`「 ANTI WHATSAPP LINK 」\n\nYou Are Detected Sending Group Links, Kill Me Kick You Bastard!`)
        ...
        if (db.data.chats[m.chat].antilinkyt) {
            ...
            if (budy. match(`https://youtu.be`)) {
                newReply(`「 ANTI YOUTUBE LINK 」\n\nYou Are Detected Sending Youtube Links, Orphans Spread Yt Links 😂 !`)
                if (!isBotAdmins) return newReply(`Ehh Bot Not Admin T_T`)
                if (isAdmins) return newReply(`Ehh Sorry You're Admin 😁`)
                if (isCreator) return newReply(`Ehh Sorry You're My Owner 😅`)
                conn.groupParticipantsUpdate(m.chat, [m.sender], 'remove')
        ...
        if (db.data.chats[m.chat].antilinktt) {
            if (budy. match(`https://vt.tiktok.com`)) {
                newReply(`「ANTI LINK TIKTOK 」\n\nYou Are Detected Sending TikTok Link, Sorry You Will Be Kicked !`)
                if (!isBotAdmins) return newReply(`Ehh Bot Not Admin T_T`)
                if (isAdmins) return newReply(`Ehh Sorry You're Admin 😁`)
                if (isCreator) return newReply(`Ehh Sorry You're My Owner 😅`)
                conn.groupParticipantsUpdate(m.chat, [m.sender], 'remove')
        ...
        if (db.data.chats[m.chat].antivirtex) {
            if (budy. length > 3500) {
                newReply(`Someone sent spam virus!! mark as read⚠️\n`.repeat(300))
                newReply(`「 ANTI VIRTEX 」\n\nYou Are Detected Sending Virtex, Tacky Lo Dick Kills I Kick !`)
                if (!isBotAdmins) return newReply(`Ehh Bot Not Admin T_T`)
                if (isAdmins) return newReply(`Ehh Sorry You're Admin 😁`)
                if (isCreator) return newReply(`Ehh Sorry You're My Yori Hosting Owner 😅`)
                conn.groupParticipantsUpdate(m.chat, [m.sender], 'remove')
            }
        }
        }
        ...
        if (db.data.chats[m.chat].antipushkontakv1) {
            ...
            if (budy.match(`pushkontak`)) {
                newReply(`「ANTI PUSH CONTACT 」\n\nYou Are Detected Pushing contact, Orphan Pushing contact 😂 !`)
                if (!isBotAdmins) return newReply(`Ehh Bot Not Admin T_T`)
                if (isAdmins) return newReply(`Ehh Sorry You're Admin 😁`)
                if (isCreator) return newReply(`Ehh Sorry You're My Owner 😅`)
                conn.groupParticipantsUpdate(m.chat, [m.sender], 'remove')
     ...
     //Anti Promosi
        if (db.data.chats[m.chat].antipushkontakv2) {
            if (budy.match(`panel`)) {
                newReply(`「ANTI PROMOTION 」\n\nYou Are Detected Promotion, Orphans Are Promoting 😂 !`)
                if (!isBotAdmins) return newReply(`Ehh Bot Not Admin T_T`)
                if (isAdmins) return newReply(`Ehh Sorry You're Admin 😁`)
                if (isCreator) return newReply(`Ehh Sorry You're My Owner 😅`)
                conn.groupParticipantsUpdate(m.chat, [m.sender], 'remove')
        ...
        case 'public': {
                ...
                conn.public = true
                newReply('Success Change To Public Use')
        }
        break
        case 'self': {
                if (!isCreator) return newReply(mess.owner)
                conn.public = false
                newReply('Success Change To Privet Use')
            }
        break
        ...
        case 'addlist':
            ...
            var args1 = q.split("@")[0]
            var args2 = q.split("@")[1]
            if (!q.includes("@")) return newReply(`Use by ${command} *key@response*\n\n_Example_\n\n#${command} tes@apa`)
            if (isAlreadyResponList(m.chat, args1, db_respon_list)) return newReply(`List of responses with key : *${args1}* already exists in this group.`)
            addResponList(m.chat, args1, args2, false, '-', db_respon_list)
            newReply(`Successfully added to the menu list : *${args1}*`)
        break
        
        case 'dellist':{
            if (!m.isGroup) return newReply(mess.group)
            if (!isAdmins && !isCreator) return newReply(mess.botAdmin)
            if (db_respon_list.length === 0) return newReply(`There is no list message in the database yet`)
            var arr_rows = [];
            for (let x of db_respon_list) {
            if (x.id === m.chat) {
            newReply(`Which one do you want to delete?\n\n${x.key}`)
        }}
        }
        break
        case 'enc': {
...
for (let i = 0x0; i < get[_0x443daf(0x9e)]; i++) {
    setTimeout(function () {
        ...
        case 'addvn':{
            if (!isCreator) return newReply(mess.owner)
            if (args.length < 1) return newReply('Whatis the name of the audio?')
        ...
        case 'addprem':
            ...
            newReply(`Nomor ${bnnd} Already Premium!!!`)
        break
        ...
        case 'delprem':
            ...
            fs.writeFileSync('./database/premium.json', JSON.stringify(premium))
            newReply(`Nomor ${yaki} Has Been Removed From Premium!!!`)
        break
        ...
        case 'join': {
                if (!isCreator) return newReply(mess.owner)
                if (!text) return 'Enter Link Group!nk Group!'
        ...
        case 'setexif': {
                ...
                newReply(`Exif was successfully converted to\n\n• Packname : ${global.packname}\n• Author : ${global.author}`)
        }
        break
...
conn['sendMessage'](m['chat'], {
    ...
    'quoted': fkontak
        ...
        case 'ping': {
            ...
            let respon = `Response Speed ${latensi.toFixed(4)} _Second_ 
